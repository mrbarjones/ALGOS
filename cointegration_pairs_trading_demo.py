"""
Cointegration Pairs Trading Strategy for CVX and XOM - DEMO VERSION
15-Minute Timeframe

This version generates synthetic cointegrated data for demonstration.
"""
...
def generate_cointegrated_series(n_periods=2000, beta=0.95, mean_reversion_speed=0.05, noise_std=0.5):
    """
    Generate two cointegrated price series that mimic CVX and XOM.
    ...
    mean_reversion_speed : float
        Speed of mean reversion for the spread
    noise_std : float
    ...
    # Generate base price series (like XOM)
    base_returns = np.random.normal(0.0001, 0.015, n_periods)
    base_price = 100 * np.exp(np.cumsum(base_returns))

    # Generate cointegrated spread
    spread = np.zeros(n_periods)
    spread[0] = 0

    for t in range(1, n_periods):
        # Mean-reverting spread
        spread[t] = spread[t-1] * (1 - mean_reversion_speed) + np.random.normal(0, noise_std)

    # Generate cointegrated price series (like CVX)
    cointegrated_price = beta * base_price + spread + 10

    # Create timestamps (15-minute intervals)
    start_date = datetime.now() - timedelta(days=60)
    timestamps = [start_date + timedelta(minutes=15*i) for i in range(n_periods)]

    # Create DataFrame
    df = pd.DataFrame({
        'CVX_close': cointegrated_price,
        'XOM_close': base_price
    }, index=timestamps)
...
class CointegrationPairsTrading:
    ...
    def load_data(self, data=None, use_synthetic=True):
        """
        Load price data (synthetic or real).
        ...
        if data is not None:
            ...
            print("Generating synthetic cointegrated data for demonstration...")
            self.data = generate_cointegrated_series()
    ...
    def run_full_analysis(self, data=None, use_synthetic=True,
        ...
        # Step 1: Load data
        self.load_data(data=data, use_synthetic=use_synthetic)
...
def main():
    ...
    # Run full analysis with synthetic data
    results = model.run_full_analysis(
        use_synthetic=True,     # Use synthetic cointegrated data
